  ...
  create(context) {
    ...
    return {
      ...
      ImportDeclaration(node): void {
          ...
          if (variable.references.length === 0) {
            ...
            const onlyHasTypeReferences = variable.references.every((ref) => {
              ...
              if (ref.isValueReference) {
                let parent: TSESTree.Node | undefined = ref.identifier.parent;
        ...
        if (
          (node.importKind === "value" && typeSpecifiers.length > 0) ||
        ...
        for (const sourceImports of sourceImportsMap.values()) {
          ...
          for (const report of sourceImports.reportValueImports) {
            if (
              ...
              const importNames = (
                isTypeImport ? report.valueSpecifiers : report.typeSpecifiers

...
interface ISourceImports {
    source: string;
    reportValueImports: IReportValueImport[];
...
export default util.createRule<TOptions, TMessageIds>({
    ...
    create(context) {
        ...
        const sourceImportsMap = new Map<string, ISourceImports>();
        ...
        return {
            ...
            ImportDeclaration(node): void {
                ...
                const sourceImports =
                    ...
                    (sourceImportsMap
                        ...
                        .get(source) as ISourceImports);

                if (node.importKind === 'type') {
                    // when import is `import type { ... } from '...' `
                    if (
                ...
                if (
                    ...
                    (node.importKind === 'type' && valueSpecifiers.length > 0)
                ) {
            ...
            'Program:exit'(): void {
                for (const sourceImports of sourceImportsMap.values()) {
                    ...
                    for (const report of sourceImports.reportValueImports) {
                        if (
                            ...
                            const message = ((): {
                                ...
                                const typeImports =
                                    util.formatWordList(importNames);
        ...
        function getFixesNamedSpecifiers(
            ...
            if (typeNamedSpecifiers.length === allNamedSpecifiers.length) {
                ...
                for (const namedSpecifiers of typeNamedSpecifierGroups) {
                    ...
                    typeNamedSpecifiersTexts.push(
                        sourceCode.text.slice(...textRange)
                    );
                }
            }
            return {
        ...
        function getNamedSpecifierRanges(
            ...
            if ((isFirst || isLast) && util.isCommaToken(after)) {
        ...
        function fixInsertNamedSpecifiersInNamedSpecifierList(
            ...
            if (
                !util.isCommaToken(before) &&
                !util.isOpeningBraceToken(before)
            ) {
        ...
        function* fixToTypeImportDeclaration(
            fixer: TSESLint.RuleFixer,
            report: IReportValueImport,
            sourceImports: ISourceImports
            ...
            if (namespaceSpecifier && !defaultSpecifier) {
                ...
                yield* fixInsertTypeSpecifierForImportDeclaration(
                    fixer,
                    node,
                    false
                );
            ...
            if (typeNamedSpecifiers.length > 0) {
                if (sourceImports.typeOnlyNamedImport) {
                    ...
                    if (
                        sourceImports.typeOnlyNamedImport.range[1] <=
                        node.range[0]
                    ) {
        ...
        function* fixInsertTypeSpecifierForImportDeclaration(
            ...
            for (const specifier of node.specifiers) {
                if (
                    ...
                    yield* fixRemoveTypeSpecifierFromImportSpecifier(
                        fixer,
                        specifier
                    );
                }
        ...
        function* fixToValueImportDeclaration(
            ...
            if (namespaceSpecifier) {
                ...
                const { valueSpecifiers } = report;
                if (
                    valueSpecifiers.includes(defaultSpecifier) &&
                    namedSpecifiers.length === 0
                ) {
                    // import type Type from 'foo'
                    yield* fixRemoveTypeSpecifierFromImportDeclaration(
                        fixer,
                        node
                    );
                    return;
            ...
            if (valueNamedSpecifiers.length > 0) {
                if (sourceImports.valueOnlyNamedImport) {
                    ...
                    if (
                        sourceImports.valueOnlyNamedImport.range[1] <=
                        node.range[0]
                    ) {
                        yield insertTypeNamedSpecifiers;
                    } else {
